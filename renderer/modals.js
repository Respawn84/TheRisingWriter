...
function openModal(modalId) {
  ...
  if (modalId === 'modal-pricing') loadPricing();
...
async function loadPricing() {
  const pricing = await window.electronAPI.getPricing();
...
async function savePricing() {
  ...
  const result = await window.electronAPI.savePricing(pricing);
  if (result.success) {
    closeModal('modal-pricing');
    showNotification('Precios guardados ✓');
  }
}
